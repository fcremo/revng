...
namespace tupletree::detail {
template<typename T>
typename std::is_same<decltype(std::declval<T>().c_str()),
                      const char *>::type
has_c_str(int);
...
template<typename T>
constexpr bool is_string_like_v = (std::is_convertible_v<std::string, T>
                                   or has_c_str_v<T>);
...
template<typename T>
constexpr bool is_container_or_tuple_v = (is_container_v<T>
                                          or has_tuple_size_v<T>);

template<typename T, typename K=void>
using enable_if_is_not_container_or_tuple_t = std::enable_if_t<!is_container_or_tuple_v<T>,
                                                               K>;
...
enable_if_has_tuple_size_t<T, void>
visit(Visitor &V, T &Obj) {
  V.preVisit(Obj);
...
enable_if_tuple_end_t<I, T, size_t>
tupleIndexByName(llvm::StringRef Name) {
  return -1;
}

template<typename T, size_t I=0>
enable_if_not_tuple_end_t<I, T, size_t>
tupleIndexByName(llvm::StringRef Name) {
...
enable_if_tuple_end_t<I, RootT, ResultT *>
...
enable_if_has_tuple_size_t<RootT, ResultT *>
getByKey(RootT &M, KeyT Key) {
  return tupletree::detail::getByKeyTuple<ResultT>(M, Key);
}

template<typename ResultT, typename RootT, typename KeyT>
enable_if_is_container_t<RootT, ResultT *>
getByKey(RootT &M, KeyT Key) {
  for (auto &Element : M) {
...
#define FE_1(ACTION, TOTAL, ARG, X)               \
  ACTION(ARG, (TOTAL) - 0, X)
...
#define INTROSPECTION_2(class, ...)                                     \
  template<int I>                                                       \
  auto &get(class &&x) {                                                \
    if constexpr (false) return NULL;                                   \
